...
| Provider-neutral bibliographic discovery foundation and shared search model | Previously missing dependency for broader consumer discovery | INITIAL AUTHOR/WORK SEARCH IMPLEMENTED / CONSUMER CUTOVER DEFERRED | MH-DISC-01 remains the production Work/Edition discovery; WISH-DISC-01 consumes it for the personal Wishlist; D-SEARCH-01 fixes the Author/Work-first target; MH-SEARCH-01A/01B now provide the parallel typed application pages populated by local canonical and Open Library search without Edition fan-out | Uses no V1 data and does not change migration targets; Works-by-Author, lazy Editions, REST/UI, shared-screen work and consumer cutovers remain separate bounded slices |
