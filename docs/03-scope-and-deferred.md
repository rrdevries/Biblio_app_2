...
| Provider-neutral bibliographic discovery foundation and shared search model | Previously missing dependency for broader consumer discovery | AUTHOR/WORK SEARCH AND SELECTED-AUTHOR WORKS REST IMPLEMENTED / CONSUMER CUTOVER DEFERRED | MH-DISC-01 remains the production Work/Edition discovery; WISH-DISC-01 consumes it for the personal Wishlist; D-SEARCH-01 fixes the Author/Work-first target; MH-SEARCH-01A/01B provide parallel typed Author/Work pages; MH-AUTHOR-API-01 exposes selector-authorized Works by one selected Author without Edition fan-out | Uses no V1 data and does not change migration targets; Work-to-Editions REST, UI/shared-screen work and consumer cutovers remain separate bounded slices |
