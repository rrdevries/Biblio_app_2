...
| Provider-neutral bibliographic discovery foundation and shared search model | Previously missing dependency for broader consumer discovery | AUTHOR/WORK SEARCH, SELECTED-AUTHOR WORKS REST AND SELECTED-WORK EDITIONS REST IMPLEMENTED / CONSUMER CUTOVER DEFERRED | MH-DISC-01 remains the production Work/Edition discovery; WISH-DISC-01 consumes it for the personal Wishlist; D-SEARCH-01 fixes the Author/Work-first target; MH-SEARCH-01A/01B provide parallel typed Author/Work pages; MH-AUTHOR-API-01 exposes selector-authorized Works by one selected Author; D-WORK-REF-01 supplies the signed Work handoff; MH-EDITION-API-01 exposes lazy Editions through that authority with current composite-mapping revalidation | Uses no V1 data and does not change migration targets; UI/shared-screen work and Wishlist/Add Book consumer cutovers remain separate bounded slices |
| Mijn Bibliotheek Grid/List, Search/Filter/Sort | V2.001 primary scope | V2.001 MUST | Users must find and manage their migrated catalog daily | Migrated active and genuine archived Items must remain discoverable under exact Library Context; the 23 manifest-bound erroneous CURRENT Copy registrations from D-MIG-COPY-EXCL-01 create no Item or user-visible record |
...
| Minimum Item/Collection archive support | V2.001 primary scope | V2.001 MUST | Genuine archived V1 product data must remain visible and manageable enough for daily use | Preserve real identity, history and archive meaning; the 23 exact erroneous CURRENT Copy registrations in D-MIG-COPY-EXCL-01 are correction evidence, not archived Items, and are excluded to durable no-target preservation |
| CURRENT erroneous archived Copy corrections | Previously assumed to need later Item/archive wiring | PRESERVE DATA ONLY | Renée explicitly decided that all 23 manifest-bound rows were wrong registrations and may not become V2 Items | Emit no Item, Archive lifecycle/reason/time, details, fulfillment, Wishlist, circulation, Reading or user-visible target; retain exactly 23 restricted reconciliation-safe observations with reason `erroneous_legacy_copy_not_carried_forward_v2` |
