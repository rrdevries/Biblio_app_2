...
| Provider-neutral bibliographic discovery foundation and shared search model | Previously missing dependency for broader consumer discovery | IMPLEMENTED FOUNDATION / TARGET MODEL CANONICAL | MH-DISC-01 safely exposes local-first Work/Edition candidates and no-Item materialization; WISH-DISC-01 consumes it for the personal Wishlist; D-SEARCH-01 now fixes one shared Author/Work-first, lazy-Edition target without implementing it | Uses no V1 data and does not change migration targets; shared full-page discovery and consumer migrations remain separate bounded slices |
