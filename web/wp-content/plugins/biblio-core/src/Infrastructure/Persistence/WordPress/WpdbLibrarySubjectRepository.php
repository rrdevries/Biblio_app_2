...
final readonly class WpdbLibrarySubjectRepository implements
    WritableLibrarySubjectRepository
{
    ...
    public function add(LibrarySubject $term): void
    {
        $this->write(fn (): int|false => $this->database->insert(
            ...
            ["%s", "%s", "%s", "%s", "%s", "%s"]
        ), "Could not persist Library Subject.");
    }
    ...
    public function rename(
        ...
        $this->write(fn (): int|false => $this->database->update(
            ...
            ["%s", "%s"],
            ["%s", "%s"]
        ), "Could not rename Library Subject.");
    }
    ...
    public function changeStatus(
        ...
        $this->write(fn (): int|false => $this->database->update(
            ...
            ["%s"],
            ["%s", "%s"]
        ), "Could not change Library Subject status.");
    }
    ...
    public function adoptSeedKey(
        ...
        $table = $this->tableNames->librarySubjects();

        return $this->write(fn (): int|false => $this->database->query(
            $this->database->prepare(
                "UPDATE `{$table}` SET seed_key = %s "
                . "WHERE library_id = %s AND subject_id = %s "
                . "AND seed_key IS NULL",
                $seedKey->value(),
                $libraryId->value(),
                $id->value()
            )
        ), "Could not adopt Library Subject seed key.", true) === 1;
    }
    ...
    private function findBy(
        ...
        $row = $this->database->get_row($this->database->prepare(
            "SELECT library_id, subject_id, display_name, normalized_name, "
        ...
        try {
            return new LibrarySubject(
                ...
                new LibrarySubjectId((string) $row->subject_id),
            ...
            throw new PersistenceException(
                "Stored Library Subject data is invalid.",
                0,
                $exception,
    ...
    private function write(
        ...
        $type = match ($constraint) {
            ...
            "subjects_by_seed_key" => ClassificationTermConflictType::SeedKey,
            default => null,
        };
        ...
        if ($type !== null) {
            throw new ClassificationTermConflict(
                $type,
                WpdbErrorTranslator::diagnostic("Subject write", $error)
            );
        }
