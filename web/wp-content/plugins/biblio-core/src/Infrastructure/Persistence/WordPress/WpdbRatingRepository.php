...
final readonly class WpdbRatingRepository implements WritableRatingRepository
{
    ...
    public function addForUser(UserId $actor, Rating $rating): void
    {
        ...
        try { $ok=$this->db->insert($this->tables->ratings(),['rating_id'=>$rating->id()->value(),'user_id'=>$rating->userId()->value(),'work_id'=>$rating->workId()->value(),'reading_round_id'=>$rating->readingRoundId()?->value(),'rating_half_units'=>$rating->value()->halfUnits(),'assessed_at'=>$rating->assessedAt()===null?null:$this->format($rating->assessedAt()),'created_at'=>$this->format($rating->createdAt()),'updated_at'=>$this->format($rating->updatedAt()),'rating_version'=>$rating->version()->value()],['%s','%s','%s','%s','%d','%s','%s','%s','%d']); } finally { $this->db->suppress_errors($old); }
    ...
    private function one(RatingId $id,UserId $user,bool $lock): ?Rating { $t=$this->tables->ratings(); $row=$this->db->get_row($this->db->prepare("SELECT rating_id,user_id,work_id,reading_round_id,rating_half_units,assessed_at,created_at,updated_at,rating_version FROM `{$t}` WHERE rating_id=%s AND user_id=%s".($lock?' FOR UPDATE':''),$id->value(),$user->value())); return $row===null?null:$this->hydrate($row); }
    /**
     * @param list<string|int> $args
     * @return list<Rating>
     */
    private function many(string $where,array $args,int $limit=100): array { $t=$this->tables->ratings(); $args[]=$limit; $rows=$this->db->get_results($this->db->prepare("SELECT rating_id,user_id,work_id,reading_round_id,rating_half_units,assessed_at,created_at,updated_at,rating_version FROM `{$t}` WHERE {$where} ORDER BY updated_at DESC,rating_id DESC LIMIT %d",...$args)); return array_map($this->hydrate(...),$rows); }
    private function hydrate(object $r): Rating { try { return new Rating(new RatingId((string)$r->rating_id),new UserId((string)$r->user_id),new WorkId((string)$r->work_id),$r->reading_round_id===null?null:new ReadingRoundId((string)$r->reading_round_id),RatingValue::fromHalfUnits((int)$r->rating_half_units),$r->assessed_at===null?null:$this->date($r->assessed_at),$this->date($r->created_at),$this->date($r->updated_at),new RatingVersion((int)$r->rating_version)); } catch(Throwable $e){ throw new PersistenceException('Stored Rating is invalid.',0,$e,FailureReason::PersistenceReadFailed); } }
