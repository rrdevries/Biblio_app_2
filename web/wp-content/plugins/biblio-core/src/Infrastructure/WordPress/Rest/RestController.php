...
final class RestController
{
    ...
    public function registerRoutes(): void
    {
        ...
        register_rest_route(
            self::NAMESPACE,
            "/libraries/(?P<library_id>[^/]+)/catalog",
            [
                "methods" => WP_REST_Server::READABLE,
        ...
        register_rest_route(self::NAMESPACE, "/me/bibliographic-searches", [
            "methods" => WP_REST_Server::CREATABLE,
        ...
        register_rest_route(self::NAMESPACE, "/me/bibliographic-work-editions", [
            "methods" => WP_REST_Server::CREATABLE,
            "callback" => [$this, "createBibliographicWorkEditionSearch"],
            "permission_callback" => [$this, "authenticated"],
        ]);
        register_rest_route(
            self::NAMESPACE,
    ...
    public function catalog(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        return $this->execute(function (
            ...
            $page = $application->catalogQuery()->query(
                $this->requests->libraryId($request),
                $this->requests->catalogQuery($request)
            );
    ...
    public function createBibliographicSearch(
        WP_REST_Request $request
    ): WP_REST_Response|WP_Error {
        return $this->execute(function (
            CoreApplication $application
        ) use ($request): WP_REST_Response {
            $result = $application->bibliographicTextSearch()->search(
                $this->requests->bibliographicTextSearch($request)
            );
    ...
    public function createBibliographicWorkEditionSearch(
        WP_REST_Request $request
    ): WP_REST_Response|WP_Error {
        return $this->execute(function (
            CoreApplication $application
        ) use ($request): WP_REST_Response {
            $page = $application->bibliographicEditionSearch()->search(
                $this->requests->bibliographicWorkEditions($request)
            );

            return $this->success(
                $this->responses->bibliographicWorkEditions($page)
            );
        });
    }
