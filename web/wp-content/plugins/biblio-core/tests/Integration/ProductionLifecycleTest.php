...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(54, $this->existingCoreTableCount());
            self::assertSame(1023, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1023));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1023, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(23, $registry->migrations());
    ...
    {
        foreach (array_reverse($this->tableNames->schema1023()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1023() as $table) {
