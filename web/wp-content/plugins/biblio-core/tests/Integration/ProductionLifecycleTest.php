...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(29, $this->existingCoreTableCount());
            self::assertSame(1011, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1011));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1011, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(11, $registry->migrations());
    ...
    public function testRuntimeRunsOnlyMissingSupportedMigration(): void
    {
        ...
        try {
            ...
            self::assertTrue($this->state->isHealthCurrent(1012));
        } finally {
            $migration->remove();
    ...
    private function ensureBaseline(): void
    {
        ...
        if ($this->existingCoreTableCount() !== 28) {
            $this->dropCoreSchema();
    ...
    {
        foreach (array_reverse($this->tableNames->schema1011()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1011() as $table) {
