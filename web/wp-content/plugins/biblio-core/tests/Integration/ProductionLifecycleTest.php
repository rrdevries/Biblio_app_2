...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(48, $this->existingCoreTableCount());
            self::assertSame(1021, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1021));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1021, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(21, $registry->migrations());
    ...
    public function testRuntimeRunsOnlyMissingSupportedMigration(): void
    {
        ...
        try {
            ...
            $migration->remove();
            update_option(CoreSchemaMigrator::VERSION_OPTION, "1021", false);
        }
    }
    ...
    private function ensureBaseline(): void
    {
        ...
        if ($this->existingCoreTableCount() !== 48) {
            $this->dropCoreSchema();
    ...
    {
        foreach (array_reverse($this->tableNames->schema1021()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1021() as $table) {
