...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(51, $this->existingCoreTableCount());
            self::assertSame(1022, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1022));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1022, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(22, $registry->migrations());
    ...
    private function ensureBaseline(): void
    {
        ...
        if ($this->existingCoreTableCount() !== 50) {
            $this->dropCoreSchema();
    ...
    {
        foreach (array_reverse($this->tableNames->schema1022()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1022() as $table) {
