...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(46, $this->existingCoreTableCount());
            self::assertSame(1018, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1018));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1018, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(18, $registry->migrations());
    ...
    public function testRuntimeRunsOnlyMissingSupportedMigration(): void
    {
        ...
        try {
            ...
            $migration->remove();
            update_option(CoreSchemaMigrator::VERSION_OPTION, "1018", false);
        }
    }
    ...
    private function ensureBaseline(): void
    {
        ...
        if ($this->existingCoreTableCount() !== 46) {
            $this->dropCoreSchema();
    ...
    {
        foreach (array_reverse($this->tableNames->schema1018()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1018() as $table) {
