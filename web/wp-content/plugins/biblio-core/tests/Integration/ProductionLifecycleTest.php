...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(40, $this->existingCoreTableCount());
            self::assertSame(1017, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1017));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1017, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(17, $registry->migrations());
    ...
    private function ensureBaseline(): void
    {
        ...
        if ($this->existingCoreTableCount() !== 40) {
            $this->dropCoreSchema();
    ...
    {
        foreach (array_reverse($this->tableNames->schema1017()) as $table) {
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1017() as $table) {
