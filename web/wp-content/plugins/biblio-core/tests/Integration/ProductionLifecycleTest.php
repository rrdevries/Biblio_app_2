...
final class ProductionLifecycleTest extends PersistenceIntegrationTestCase
{
    ...
    public function testActivationInstallsAndValidatesFreshBaseline(): void
    {
        ...
        try {
            ...
            self::assertSame(28, $this->existingCoreTableCount());
            self::assertSame(1010, $migrator->installedVersion());
            self::assertTrue(
                $migrator->health()->isHealthy(),
                $migrator->health()->summary()
            );
            self::assertTrue($this->state->isHealthCurrent(1010));
        } finally {
            $this->ensureBaseline();
        }
    }
    ...
    public function testProductionRegistryMatchesCurrentBaselineVersion(): void
    {
        ...
        self::assertSame(1010, CoreSchemaMigrator::CURRENT_VERSION);
        self::assertCount(10, $registry->migrations());
    ...
    private function existingCoreTableCount(): int
    {
        ...
        foreach ($this->tableNames->schema1010() as $table) {
